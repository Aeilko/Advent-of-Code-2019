...
public class Solution {
	...
	public static TreeSet<Coordinate> getPath(String[] in){
		...
		for(String move: in){
			...
			if(direction == 'U'){
				...
				for(int i = 0; i < l; i++){
					cur = new Coordinate(cur.getX()+1, cur.getY());
					result.add(cur);
				}
			}
			else if(direction == 'L'){
	...
	public static TreeMap<Coordinate, Integer> getPathPart2(String[] in){
		TreeMap<Coordinate, Integer> result = new TreeMap<>();
	...
	public static void main(String[] args) {
		...
		TreeSet<Coordinate> pathA = getPath(a);

		String[] b = inputB.split(",");
		TreeSet<Coordinate> pathB = getPath(b);

		// Calculate intersection of pathA and pathB
		pathA.retainAll(pathB);

		// Find the closest coordinate
		int dist = Integer.MAX_VALUE;
		for(Coordinate c: pathA){
			int d = Math.abs(c.getX())+Math.abs(c.getY());
			if(d < dist)
				dist = d;
		}
		...
		Set<Coordinate> intersect = pA.keySet();
		intersect.retainAll(pB.keySet());
