package day19;

import utils.Grids.Coordinate;
import utils.IntcodeComputer.IntcodeComputer;

import java.util.concurrent.ArrayBlockingQueue;

public class Solution {

	public static final String PROGRAM = "109,424,203,1,21102,1,11,0,1106,0,282,21101,0,18,0,1106,0,259,1202,1,1,221,203,1,21101,0,31,0,1105,1,282,21102,38,1,0,1105,1,259,20102,1,23,2,21201,1,0,3,21102,1,1,1,21101,0,57,0,1105,1,303,2101,0,1,222,20102,1,221,3,21002,221,1,2,21101,0,259,1,21101,0,80,0,1106,0,225,21102,1,152,2,21101,91,0,0,1106,0,303,1201,1,0,223,21001,222,0,4,21101,0,259,3,21102,225,1,2,21101,0,225,1,21102,1,118,0,1105,1,225,20101,0,222,3,21102,61,1,2,21101,133,0,0,1106,0,303,21202,1,-1,1,22001,223,1,1,21102,148,1,0,1105,1,259,2101,0,1,223,21001,221,0,4,21001,222,0,3,21101,0,14,2,1001,132,-2,224,1002,224,2,224,1001,224,3,224,1002,132,-1,132,1,224,132,224,21001,224,1,1,21101,0,195,0,105,1,109,20207,1,223,2,20101,0,23,1,21102,-1,1,3,21102,214,1,0,1105,1,303,22101,1,1,1,204,1,99,0,0,0,0,109,5,2101,0,-4,249,21202,-3,1,1,21202,-2,1,2,21201,-1,0,3,21102,1,250,0,1106,0,225,22101,0,1,-4,109,-5,2106,0,0,109,3,22107,0,-2,-1,21202,-1,2,-1,21201,-1,-1,-1,22202,-1,-2,-2,109,-3,2105,1,0,109,3,21207,-2,0,-1,1206,-1,294,104,0,99,22102,1,-2,-2,109,-3,2105,1,0,109,5,22207,-3,-4,-1,1206,-1,346,22201,-4,-3,-4,21202,-3,-1,-1,22201,-4,-1,2,21202,2,-1,-1,22201,-4,-1,1,21202,-2,1,3,21101,343,0,0,1106,0,303,1105,1,415,22207,-2,-3,-1,1206,-1,387,22201,-3,-2,-3,21202,-2,-1,-1,22201,-3,-1,3,21202,3,-1,-1,22201,-3,-1,2,22101,0,-4,1,21101,0,384,0,1106,0,303,1105,1,415,21202,-4,-1,-4,22201,-4,-3,-4,22202,-3,-2,-2,22202,-2,-4,-4,22202,-3,-2,-3,21202,-4,-1,-2,22201,-3,-2,1,21201,1,0,-4,109,-5,2106,0,0";

	public static void main(String[] args){

		long[] stack = IntcodeComputer.processInput(PROGRAM);

		boolean[][] grid = checkGrid(2000, stack);

		int result = 0;
		for(int x = 0; x < 50; x++){
			for(int y = 0; y < 50; y++){
				if(grid[x][y])
					result++;
			}
		}

		System.out.println("Part 1 - Success");
		System.out.println("Answer\t" + result + "\n");

		Coordinate c = findSquare(grid);

		if(c == null){
			System.out.println("Part 2 - Failed");
			System.out.println("Could not find the grid");
		}
		else{
			System.out.println("Part 2 - Success");
			System.out.println("Answer\t" + (c.getX()*10000 + c.getY()));
		}

	}

	public static long checkCoordinate(long x, long y, long[] stack){
		IntcodeComputer ic = new IntcodeComputer(stack, 2);
		ArrayBlockingQueue<Long> in = new ArrayBlockingQueue<>(64);
		ArrayBlockingQueue<Long> out = new ArrayBlockingQueue<>(64);
		ic.setInputArray(in);
		ic.setOutputArray(out);
		long result = -1;
		try {
			in.add(x);
			in.add(y);
			ic.run();
			result = out.take();
		}
		catch (Exception e) {
			e.printStackTrace();
		}

		return result;
	}

	public static boolean[][] checkGrid(int gridSize, long[] stack){
		boolean[][] result = new boolean[gridSize][gridSize];

		int prevStart = 0;
		int prevLength = 1;

		for(int y = 0; y < gridSize; y++){
			int curStart = -1;
			for(int x = prevStart; x <= prevStart+prevLength+2; x++){
				long r = checkCoordinate(x, y, stack);
				if(r == 1){
					result[x][y] = true;
					if(curStart == -1)
						curStart = x;
				}
				else if(r == 0 && curStart != -1){
					// Found something outside the beam after we already discovered something inside the beam on this line
					prevStart = curStart;
					prevLength = x-curStart;
					break;
				}
			}
		}

		return result;
	}

	public static Coordinate findSquare(boolean[][] grid){
		int coordX = -1;
		int coordY = -1;

		int prevStart = 0;
		outer: for(int y = 0; y < grid[0].length; y++){
			int curStart = -1;
			for(int x = prevStart; x < grid[0].length; x++){
				if(grid[x][y]){
					if(curStart == -1)
						curStart = x;

					if(grid[x+99][y] && grid[x][y+99]){
						coordX = x;
						coordY = y;
						break outer;
					}
				}
				else{
					if(curStart != -1)
						break;;
				}
			}

			if(curStart != -1)
				prevStart = curStart;
		}

		Coordinate result = null;
		if(coordX != -1)
			result = new Coordinate(coordX, coordY);
		return result;
	}

	public static void printGrid(boolean[][] in){
		for(int y = 0; y < in[0].length; y++){
			for(int x = 0; x < in.length; x++){
				System.out.print(in[x][y] ? "#" : " ");
			}
			System.out.println();
		}
	}
}
